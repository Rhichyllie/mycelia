...
| Domain Model | 03 | DESCRIPTOR_CONTRACTS_AND_SCHEMA_CONTRACT_IMPLEMENTED_PARTIAL | `src/mycelia/*` descriptor modules, `src/mycelia/runtime-persistence-model/`, `src/mycelia/minimal-persistence-activation/`, `prisma/schema.prisma` | Current source defines typed descriptors, a persistence scaffold and a schema/migration contract; no repository layer or application DB reads/writes exist yet. |
...
| State/Persistence | 06 | SCHEMA_MIGRATION_CONTRACT_ACTIVE_REPOSITORY_NOT_IMPLEMENTED | `src/mycelia/runtime-state/`, `src/mycelia/state-transition/`, `src/mycelia/state-transition-coordinator/`, `src/mycelia/runtime-persistence-model/`, `src/mycelia/governed-run-lifecycle/`, `src/mycelia/minimal-persistence-activation/`, `prisma/schema.prisma` | Minimal persistence record scaffold, pure lifecycle logic and schema/migration contract exist; no repository layer, application DB reads/writes or checkpoints exist yet. |
...
| Product/Commercial Planning | 00, 01, 19 | INTERNAL_PLANNING_AND_MINIMAL_SCHEMA_CONTRACT_IMPLEMENTED | `docs/product/`, `src/mycelia/initial-use-case-freeze/`, `src/mycelia/pilot-offer-package/`, `src/mycelia/runtime-slice-technical-plan/`, `src/mycelia/governed-run-lifecycle/`, `src/mycelia/policy-admission-v1/`, `src/mycelia/audit-commit-boundary/`, `src/mycelia/approval-gate-v1/`, `src/mycelia/investigation-view-model-v1/`, `src/mycelia/replay-dry-run-descriptor-v1/`, `src/mycelia/internal-runtime-orchestrator-v1/`, `src/mycelia/runtime-slice-consistency-audit/`, `src/mycelia/minimal-persistence-activation/` | Pricing ranges are internal planning assumptions, not guarantees; pure runtime-slice layers remain in memory and Phase 3A adds only the schema/migration contract. |
...
| Investigation/Replay UX | planned | PURE_INVESTIGATION_AND_REPLAY_DESCRIPTOR_ONLY | `src/mycelia/investigation-view-model-v1/`, `src/mycelia/replay-dry-run-descriptor-v1/`, future `investigation-replay-ux` | Static walkthrough, pure investigation view model and replay dry-run descriptor exist; real investigation/replay UX, database reads and replay execution do not. |
