...
`src/mycelia/persistence/runtime-persistence-model/` defines readonly record descriptors and pure Zod schemas for the six first-slice records.
...
- GovernedRun maps to `src/mycelia/domain-contracts/governed-run/`.
- RuntimeStateSnapshot maps to `src/mycelia/domain-contracts/runtime-state/` and `src/mycelia/domain-contracts/state-transition/`.
- PolicyDecisionRecord maps to `src/mycelia/domain-contracts/policy-decision-gateway/`.
- AdmissionDecisionRecord maps to `src/mycelia/domain-contracts/runtime-admission-gateway/`.
- ApprovalRequest maps to `src/mycelia/runtime-logic/approval-gate-v1/` plus policy/admission records.
- AuditRecord maps to `src/mycelia/domain-contracts/audit-record/` and `src/mycelia/domain-contracts/audit-recorder/`.
- Investigation view references map to `src/mycelia/runtime-logic/investigation-view-model-v1/` for descriptor assembly only.
- Replay dry-run references map to `src/mycelia/runtime-logic/replay-dry-run-descriptor-v1/` for descriptor assembly only.
- Internal orchestration references map to `src/mycelia/runtime-logic/internal-runtime-orchestrator-v1/` for in-memory descriptor composition only.
