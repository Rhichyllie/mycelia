...
function authSessionUser(session: Session): SessionUserWithAuthClaims | undefined {
  return session.user as SessionUserWithAuthClaims | undefined;
}
...
export function requireSessionUserId(session: Session): string {
  const userId = authSessionUser(session)?.id?.trim();
