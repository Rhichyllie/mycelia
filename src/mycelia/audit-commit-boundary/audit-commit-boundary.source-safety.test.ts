...
const auditCommitBoundarySourcePath = new URL(
  "./audit-commit-boundary.ts",
  import.meta.url,
);

const FORBIDDEN_AUDIT_COMMIT_BOUNDARY_SOURCE_PATTERNS = [
...
describe("audit commit boundary source safety", () => {
  it("does not add runtime, browser, route, file or external service behavior", () => {
    const moduleSource = source(auditCommitBoundarySourcePath).toLowerCase();

    for (const pattern of FORBIDDEN_AUDIT_COMMIT_BOUNDARY_SOURCE_PATTERNS) {
      expect(moduleSource).not.toContain(pattern.toLowerCase());
    }
  });

  it("does not add forms, action controls or external links", () => {
    const moduleSource = source(auditCommitBoundarySourcePath);
